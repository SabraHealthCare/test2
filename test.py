...
def Initial_Mapping(operator):
    ...
    account_mapping['Sabra_Account'] = account_mapping['Sabra_Account'].apply(lambda x: x.upper() if x is not None or x !=np.nan else x)
    account_mapping['Sabra_Second_Account'] = account_mapping['Sabra_Second_Account'].apply(lambda x: x.upper() if x is not None or x is not " " else x)
