...
def Initial_Mapping(operator):
    ...
    account_mapping.loc['Sabra_Account'] = account_mapping.loc['Sabra_Account'].apply(lambda x: x.upper() if (x is not None) and pd.notna(x) else x)
    account_mapping.loc['Sabra_Second_Account'] = account_mapping.loc['Sabra_Second_Account'].apply(lambda x:  x.upper() if (x is not None) and pd.notna(x) else x)
...
def Map_PL_Sabra(PL,entity):
    ...
    #second_account_mapping["Sabra_Second_Account"]=account_mapping["Sabra_Second_Account"].apply(lambda x:x.strip() if (x is not None) & ( pd.notna(x)) else x)
    second_account_mapping=account_mapping.dropna(sebset=["Sabra_Second_Account"])
